  ...
  return (
    <footer className="border-t border-border bg-surface py-12">
      <div className="container grid gap-8 md:grid-cols-[2fr_1fr_1fr_1fr]">
        ...
        <div className="grid gap-2 text-sm text-muted-foreground"><p className="font-bold text-foreground">{dictionary.nav.company}</p><Link href={localizedPath(locale, "/about")}>{dictionary.nav.about}</Link><Link href={localizedPath(locale, "/news")}>{dictionary.nav.news}</Link><Link href={localizedPath(locale, "/awards")}>{dictionary.nav.awards}</Link><Link href={localizedPath(locale, "/blog")}>{dictionary.nav.blog}</Link><Link href={localizedPath(locale, "/contact")}>{dictionary.footer.contactSupport}</Link></div>
